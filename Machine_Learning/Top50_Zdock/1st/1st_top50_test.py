...
data_test_path=os.path.join(current_dir, '1st_top50_all.csv')
...
def data_handling(df_trainning_set,df_testing_set):
    ...
    X_Test = df_testing_set.iloc[:,1:6].values
    Y_Test = df_testing_set.iloc[:, 6].values
    # Feature Scaling
    sc_X = StandardScaler()
...
def plot_data(y_test,y_pred):
    ...
    ax1=sns.heatmap(cm, annot=True,cbar=False,
                cmap='Set2',annot_kws={'size':50},fmt=".0f")
    ...
    plt.tight_layout()
    # plt.savefig('1st_round_top50.tiff',dpi=300)
    plt.show()
