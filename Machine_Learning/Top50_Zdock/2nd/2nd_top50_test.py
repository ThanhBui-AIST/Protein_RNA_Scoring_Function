...
data_test_path=os.path.join(current_dir, '2nd_top50_all.csv')
...
def data_handling(df_trainning_set,df_testing_set):
    ...
    X_Test = df_testing_set.iloc[:,1:4].values
    Y_Test = df_testing_set.iloc[:, 4].values
    # Feature Scaling
    sc_X = StandardScaler()
...
def plot_data(y_test,y_pred):
    ...
    plt.tight_layout()
    # plt.savefig('2nd_round_top50.tiff',dpi=300)
    plt.show()
